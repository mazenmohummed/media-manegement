...
export async function GET(
  ...
  try {
    ...
    const employee = await prisma.user.findUnique({
      ...
      include: {
        ...
        attendanceLogs: {
          take: 30,
          orderBy: { date: "desc" },
          // Include the task relationship and its nested project parent
          include: {
            task: {
              include: {
                project: true,
    ...
    // Server-side calculated metrics based on the actual schema fields
    const totalGrossRevenue = employee.tasks.reduce((sum, t) => sum + (t.internalCost || 0), 0);

    // Net Agency Profit: summing pre-calculated marginAmount per task
...
export async function PUT(
  ...
  try {
    ...
    // NOTE: the schema field is `baseSalary`, not `salary` — using the wrong
    // key here throws a Prisma "Unknown argument" error on every save.
    const updateData: any = {
      name: body.name,
      email: body.email,
      role: body.role,
      userType: body.userType,
      // Wrap in Math.abs to strip out negative numbers from manual input
      baseSalary: Math.abs(parseFloat(body.baseSalary)) || 0,
      // Only touch efficiencyRate if the client actually sent one — otherwise
      // every save would silently reset it back to the 1.0 default.
      efficiencyRate:
        body.efficiencyRate !== undefined && body.efficiencyRate !== null && body.efficiencyRate !== ""
          ? parseFloat(body.efficiencyRate)
          : existingEmployee.efficiencyRate,
