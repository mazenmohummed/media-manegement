...
export const GET = withAuthGuard(
  "project:read",
  async (req: Request, { agencyId }, context) => {
    try {
...
export const PATCH = withAuthGuard(
  ...
  async (req: Request, { agencyId }, context) => {
    try {
      ...
      // ✅ `campaignId` removed — it's not a scalar on Project
      const allowedFields = [
        "name",
        "projectName",
        "status",
      ...
      // ✅ Handle campaignId via the many-to-many relation
      if (body.campaignId !== undefined) {
        if (body.campaignId) {
          const campaign = await db.campaign.findFirst({
            where: { id: body.campaignId, agencyId },
            select: { id: true },
          });
          if (!campaign) {
            return NextResponse.json(
          ...
          updateData.campaigns = { set: [{ id: body.campaignId }] };
        } else {
          updateData.campaigns = { set: [] };
        }
      }
      ...
      if (updateData.clientId) {
        const client = await db.client.findFirst({
          where: { id: updateData.clientId, agencyId },
          select: { id: true },
        });
        if (!client) {
          return NextResponse.json(
      ...
      if (updateData.contractId) {
        const contract = await db.contract.findFirst({
          where: { id: updateData.contractId, agencyId },
          select: { id: true },
        });
        if (!contract) {
          return NextResponse.json(
  ...
  async (req: Request, { agencyId }, context) => {
    try {
      ...
      if (targetDeadline !== undefined) {
        updateData.targetDeadline = targetDeadline
      ...
      // ✅ Handle campaignId via the relation
      if (campaignId !== undefined) {
        if (campaignId) {
          ...
          updateData.campaigns = { set: [{ id: campaignId }] };
        } else {
          updateData.campaigns = { set: [] };
        }
      }
        ...
        if (tagIds.length > 0) {
          ...
          if (tags.length !== tagIds.length) {
            return NextResponse.json(
              {
                error:
                  "One or more tags not found or do not belong to your agency",
              },
              { status: 400 }
            );
          }
    ...
    try {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Cannot delete an active project with tasks. Archive it instead.",
