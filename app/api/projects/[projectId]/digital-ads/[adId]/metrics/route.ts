...
export async function GET(
  ...
  try {
    ...
    const enrichedMetrics = metrics.map((metric) => ({
      ...metric,
      ctr: metric.ctr ?? (metric.impressions > 0 ? (metric.clicks / metric.impressions) * 100 : 0),
      cpc: metric.cpc ?? (metric.clicks > 0 ? metric.spend / metric.clicks : 0),
      cpm: metric.cpm ?? (metric.impressions > 0 ? (metric.spend / metric.impressions) * 1000 : 0),
      roas: metric.roas ?? (metric.spend > 0 ? metric.revenue / metric.spend : 0),
    }));
...
export async function POST(
  ...
  try {
    ...
    const ctr = validatedData.ctr ?? (validatedData.impressions > 0 ? (validatedData.clicks / validatedData.impressions) * 100 : 0);
    const cpc = validatedData.cpc ?? (validatedData.clicks > 0 ? validatedData.spend / validatedData.clicks : 0);
    const cpm = validatedData.cpm ?? (validatedData.impressions > 0 ? (validatedData.spend / validatedData.impressions) * 1000 : 0);
    const roas = validatedData.roas ?? (validatedData.spend > 0 ? validatedData.revenue / validatedData.spend : 0);
