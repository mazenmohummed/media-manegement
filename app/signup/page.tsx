      ...
      <div className="bg-card p-10 md:p-12 rounded-[2.5rem] shadow-2xl w-full max-w-2xl border border-border relative z-10">
        <header className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em]">System Initialization</span>
          </div>
          <h2 className="text-4xl font-black text-foreground mb-3 tracking-tight uppercase">
            Deploy Agency Space
          </h2>
          <p className="text-muted-foreground font-medium max-w-md">
            Initialize the neural system for your media agency operations.
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
          ...
          <button 
            disabled={loading}
            className="col-span-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all active:scale-[0.98] shadow-xl shadow-blue-500/20 mt-4"
          >
            {loading ? "Initializing..." : "Establish Agency Terminal"}
          </button>
        </form>
