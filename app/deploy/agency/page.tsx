...
export default function AgencySignup() {
  ...
  const handleNext = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Save to sessionStorage to persist between steps
    sessionStorage.setItem("pending_agency", JSON.stringify(agencyData));
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-6">
      <div className="bg-card p-10 rounded-[2.5rem] border border-border w-full max-w-lg shadow-2xl">
        <div className="flex items-center gap-3 mb-8">
          ...
          <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Step 01: Brand Identity</span>
        </div>
