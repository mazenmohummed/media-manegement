...
export async function archiveOldProjectsJob() {
  ...
  try {
    // ✅ Get ArchiveService with dependencies
    ...
    for (const agency of agencies) {
      try {
        ...
        results.totalArchived += result.archived || 0;
        results.success++;
        ...
        console.log(`✅ Archived ${result.archived || 0} files for ${agency.agencyName}`);
    ...
    console.log(`📊 Archive job completed: ${results.success} succeeded, ${results.failed} failed`);
