...
import { MessageSquare, Loader2, Trash2, AtSign, X, User } from "lucide-react";
...
interface ConceptCommentsTabProps {
  projectId: string;
  conceptId: string;
}

export function ConceptCommentsTab({ projectId, conceptId }: ConceptCommentsTabProps) {
  ...
  const loadComments = () =>
    fetch(`/api/projects/${projectId}/concepts/${conceptId}/comments`)
      .then((r) => r.json())
